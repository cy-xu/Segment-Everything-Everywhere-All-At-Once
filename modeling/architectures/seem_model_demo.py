...
class GeneralizedSEEM(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        return {
            ...
            "test_topk_per_image": cfg["MODEL"]["DECODER"]["TEST"][
    ...
    def forward(self, batched_inputs, mode="default"):
        if self.training:
            ...
            if self.task_switch["openimage"] and self.task_switch["openimage"]["mask"]:
                ...
                losses_openimage = {
                    key.replace("mask", "openimage"): value
                    for key, value in losses_openimage.items()
                }
                losses_openimage = {
                    key.replace("grounding", "grounding_openimage"): value
                    for key, value in losses_openimage.items()
                }
    ...
    def forward_seg(self, batched_inputs):
        ...
        self.sem_seg_head.predictor.lang_encoder.get_text_embeddings(
            self.train_class_names, is_eval=False
        )
        ...
        if "instances" in batched_inputs[0]:
            ...
            if self.task_switch["grounding"]:
                ...
                grounding_tokens = nn.utils.rnn.pad_sequence(
                    grounding_tokens, padding_value=-1
                )
                non_zero_query_mask = (
                    grounding_tokens.sum(dim=-1) == -grounding_tokens.shape[-1]
                )
            ...
            if self.task_switch["spatial"]:
                pos_masks = [
                    x["spatial_query"]["rand_shape"].to(self.device)
                    for x in batched_inputs
                ]
                neg_masks = [
                    (x["spatial_query"]["rand_shape"].to(self.device) & False)
                    for x in batched_inputs
                ]
        ...
        extra = {
            ...
            "class_embeddings": getattr(
                ...
                "{}_text_embeddings".format("default"),
            ),
    ...
    def evaluate_demo(self, batched_inputs):
        ...
        if "stroke" in batched_inputs[0]:
            ...
            pos_masks = ImageList.from_tensors(
                pos_masks, self.size_divisibility
            ).tensor.unbind(0)
            neg_masks = (batched_inputs[0]["stroke"].to(self.device) & False).unbind(0)
            neg_masks = ImageList.from_tensors(
                neg_masks, self.size_divisibility
            ).tensor.unbind(0)
            extra.update(
                {
                    "spatial_query_pos_mask": pos_masks,
        ...
        pos_masks = (
            batched_inputs[0]["spatial_query"]["rand_shape"].to(self.device)
        ).unbind(0)
        pos_masks = ImageList.from_tensors(
            pos_masks, self.size_divisibility
        ).tensor.unbind(0)
        ...
        neg_masks = (
            batched_inputs[0]["spatial_query"]["rand_shape"].to(self.device) & False
        ).unbind(0)
        neg_masks = ImageList.from_tensors(
            neg_masks, self.size_divisibility
        ).tensor.unbind(0)
        extra.update(
            {"spatial_query_pos_mask": pos_masks, "spatial_query_neg_mask": neg_masks}
        )
        ...
        for i in range(self.interactive_iter):
            ...
            extra.update(outputs)
            pred_smask = F.interpolate(
                outputs["prev_mask"], images.tensor.shape[-2:], mode="bicubic"
            )
    ...
    def evaluate_grounding(self, batched_inputs, mode):
        ...
        assert (
            len(images.tensor) == 1
        ), "grounding evaluation only support single batch size now"
    ...
    def evaluate_grounding_sptial(self, batched_inputs, mode):
        ...
        pos_masks = (
            F.conv2d(pos_masks.float(), self.dilation_kernel, padding=dilation // 2) > 0
        ).unbind(0)
        ...
        for idx, batch_per_image in enumerate(batched_inputs):
            ...
            for idx2, anno_text in enumerate(grd_texts):
                extra.update(
                    {
                        "spatial_query_pos_mask": [pos_masks[idx2]],
                ...
                grd_emb = token_emb[0][tokens["attention_mask"].bool()[0]]
                non_zero_query_mask = torch.zeros(
                    grd_emb[:, None].shape[:-1], dtype=torch.bool, device=grd_emb.device
                )
                extra["grounding_tokens"] = grd_emb[:, None]
                extra["grounding_nonzero_mask"] = non_zero_query_mask.t()
                ...
                outputs = self.sem_seg_head(
                    features, extra=extra, task="grounding_eval"
                )
        ...
        for idx, batch_per_image in enumerate(batched_inputs):
            ...
            if self.task_switch["grounding"]:
                ...
                if len(grd_masks) == 0:
                    ...
                    padded_masks = torch.zeros(
                        (grd_masks.shape[0], h_pad, w_pad),
                        dtype=grd_masks.dtype,
                        device=grd_masks.device,
                    )
                    padded_masks[
                        :, : grd_masks.shape[1], : grd_masks.shape[2]
                    ] = grd_masks
                ...
                class_idx = torch.stack(
                    (torch.arange(len(class_idx), device=class_idx.device), class_idx)
                ).tolist()
    ...
    def prepare_next_spaital_mask(self, outputs, batched_inputs):
        gt_masks = [
            batched_inputs[i]["spatial_query"]["gt_masks"]
        ...
        if self.training:
            ...
            gt_masks = ImageList.from_tensors(
                gt_masks, self.size_divisibility
        ...
        fn = (
            gt_masks & (~(gt_masks & pred_masks)) & (~prev_masks)
        )  # fn: False Negative, gt:1, pred:0, prev:0
        fp = (~gt_masks & pred_masks) & (
            ~prev_masks
        )  # fp: False Positive, gt:0, pred:1, prev:0
        ...
        select_mask = torch.stack(
            [fn[i] if is_postive[i] else fp[i] for i in range(len(fn))]
        )
        ...
        mask_dt = (
            distance_transform(
                (
                    ~F.pad(select_mask, pad=(1, 1, 1, 1), mode="constant", value=0)
                ).float()
            )[:, :, 1:-1, 1:-1]
        ).reshape(n, -1)
        max_xy_idx = torch.stack(
            [torch.arange(n), mask_dt.max(dim=-1)[1].cpu()]
        ).tolist()
        next_mask = torch.zeros(
            gt_masks.shape, device=torch.cuda.current_device()
        ).bool()
        ...
        return {
            "spatial_query_pos_mask": pos_mask,
            "spatial_query_neg_mask": neg_mask,
            "false_positive_mask": fp,
        }
