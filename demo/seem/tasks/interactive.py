...
colors_list = [
    (np.array(color["color"]) / 255).tolist() for color in COCO_CATEGORIES
] + [[1, 1, 1]]
...
def interactive_infer_image(
    ...
    if "Example" in tasks:
        ...
        batched_inputs = [
            {"image": images, "height": _height, "width": _width, "spatial_query": {}}
        ]
    ...
    if "Panoptic" in tasks:
        ...
        if "clip_seem" in tasks:
            return results

        demo = visual.draw_panoptic_seg(pano_seg.cpu(), pano_seg_info)  # rgb Image
        res = demo.get_image()

        # temp_img = Image.fromarray(res)
        # temp_img = temp_img.resize((width, height))
    ...
    for idx, mask in enumerate(pred_masks_pos):
        ...
        demo = visual.draw_binary_mask(
            mask, color=colors_list[pred_class[0] % 133], text=out_txt
        )
    res = demo.get_image()
    ...
    if "Video" in tasks:
        ...
        for frame_pth in frame_pths:
            ...
            pred_masks_pos = (
                (
                    F.interpolate(
                        pred_masks_pos[None,], image_size[-2:], mode="bilinear"
                    )[0, :, : data["height"], : data["width"]]
                    > 0.0
                )
                .float()
